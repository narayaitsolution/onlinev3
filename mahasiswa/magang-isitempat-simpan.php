...
//masukin data
$stmt = $dbsurat->prepare("INSERT INTO magang (tanggal, prodi, nim, nama, instansi, tempatpkl, alamat, tglmulai, tglselesai, pklmagang, jenispkl,token) 
                            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
...
$qpkl = mysqli_query($dbsurat, "SELECT * FROM magang WHERE nim='$nim' and statussurat=-1");
$dpkl = mysqli_fetch_array($qpkl);
$nodata = $dpkl['no'];

header("location:magang-isianggota.php?nodata=$nodata");
