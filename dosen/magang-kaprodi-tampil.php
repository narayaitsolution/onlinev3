...
<body class="hold-transition sidebar-mini text-sm">
    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper">
            ...
            $stmt = $dbsurat->prepare("SELECT * FROM magang WHERE token=?");
            $stmt->bind_param("s", $token);
            $stmt->execute();
            $result = $stmt->get_result();
            $dmagang = $result->fetch_assoc();
            $nodata = $dmagang['no'];
            $namamhs = $dmagang['nama'];
            $nimmhs = $dmagang['nim'];
            $instansi = $dmagang['instansi'];
            $tempatmagang = $dmagang['tempatpkl'];
            $alamat = $dmagang['alamat'];
            $tglmulai = $dmagang['tglmulai'];
            $tglselesai = $dmagang['tglselesai'];
            $pklmagang = $dmagang['pklmagang'];
            $jenismagang = $dmagang['jenispkl'];
            $lampiran = $dmagang['lampiran'];
            $validator1 = $dmagang['validator1'];
            $tglvalidasi1 = $dmagang['tglvalidasi1'];
            $validasi1 = $dmagang['validasi1'];
                                ...
                                <div class="card-body p-0">
                                    <div class="card-body">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="tempatmagang" name="tempatmagang" value="<?= $tempatmagang; ?>" readonly>
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="jenismagang" name="jenismagang" value="<?= $jenismagang; ?>" readonly>
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                        ...
                                        <div class="form-group row">
                                            <label for="paktaintegritas" class="col-sm-2 col-form-label">Anggota Magang</label>
                                            <div class="col-sm-10">
                                                <table id="example2" class="table table-bordered table-hover text-sm">
                                                    ...
                                                    <tbody>
                                                        <?php
                                                        $no = 1;
                                                        $stmt = $dbsurat->prepare("SELECT * FROM maganganggota WHERE nodata=?");
                                                        $stmt->bind_param("s", $nodata);
                                                        $stmt->execute();
                                                        $result = $stmt->get_result();
                                                        while ($dmagang = $result->fetch_assoc()) {
                                                            $nimanggota = $dmagang['nimanggota'];
                                                            $namaanggota = $dmagang['nama'];
                                                            $buktivaksin = $dmagang['buktivaksin'];
                                                        ?>
                                                            <tr>
                                                                <td><?= $no; ?></td>
                                        ...
                                        <form role="form" method="POST">
                                            ...
                                            <div class="row">
                                                <div class="col-6">
                                                    <button name="aksi" value="setujui" type="submit" formaction="magang-kaprodi-setujui.php" class="btn btn-success btn-block btn-lg" onclick="return confirm('Apakah anda menyetujui pengajuan ini ?')"> <i class="fa fa-check"></i> Setujui</button>
                                                </div>
                                                <div class="col-6">
                                                ...
                                                <div class="modal-dialog modal-lg">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer justify-content-between">
                                                            ...
                                                            <button name="aksi" value="tolak" type="submit" formaction="magang-kaprodi-tolak.php" class="btn btn-danger btn-sm" onclick="return confirm('Apakah anda yakin akan menolak pengajuan ini ?')"> <i class="fa fa-times"></i> Tolak</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
