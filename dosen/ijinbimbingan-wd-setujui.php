...
$tglselesai = date('Y-m-d', strtotime("+3 months", strtotime($tglmulai)));
...
$qurutan = mysqli_query($dbsurat, "SELECT * FROM ijinbimbingan WHERE year(tanggal)=$tahun");
$urutan = mysqli_num_rows($qurutan);

$nosurat = "B-" . $urutan . ".O/FST.01/TL.00/" . $bulan . "/" . $tahun . "";

//update status validasi kaprodi
$sql = mysqli_query($dbsurat, "UPDATE ijinbimbingan
...
$sql1 = mysqli_query($dbsurat, "SELECT * FROM ijinbimbingan WHERE token='$token'");
...
$pesan = "Yth. " . $namamhs . "<br/>
        ...
        <br/>
        <a href='https://eoffice.saintek.uin-malang.ac.id/mahasiswa/ijinpenelitian-cetak.php?token=$token' style=' background-color: #0045CE;border: none;color: white;padding: 8px 16px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;'>Cetak Surat</a><br/>
        <br/>
